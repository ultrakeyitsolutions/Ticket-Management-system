...
# Test 4: Test POST request to create admin with unique username
print('\n4. Testing admin creation with unique username...')
try:
    import random
...
try:
    ...
    test_user = User.objects.filter(username=unique_username).first()
    if test_user:
        ...
        company = Company.objects.filter(name=f'{unique_username} Company').first()
        if company:
...
print('2. Fill form with unique username and valid email')
print('3. Click submit')
print('4. Check browser console (F12) for errors')
print('5. Check network tab for request/response')

print('\n=== EXPECTED BEHAVIOR ===')
print('1. Form should validate fields')
print('2. User should be created with is_staff=True')
print('3. Company should be created')
print('4. Trial subscription should be created')
...
print('6. Check if username/email already exists')
