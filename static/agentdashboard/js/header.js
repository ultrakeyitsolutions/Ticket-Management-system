...
window.initializeHeader = function() {
    ...
    if (notificationsBtn && notificationsDropdown) {
        notificationsBtn.addEventListener('click', function(e) {
            ...
            if (notificationsDropdown.classList.contains('show')) {
                window.__refreshAgentNotifications && window.__refreshAgentNotifications();
            }
        });
    }
    ...
    if (window.__refreshAgentNotifications) {
        window.__refreshAgentNotifications();
        setInterval(window.__refreshAgentNotifications, 20000);
    }
...
async function refreshAgentNotifications() {
    ...
    const list = document.getElementById('agentNotificationsList');
    const empty = document.getElementById('agentNotificationsEmpty');

    if (!badge || !list || !empty) return;

    try {
        const res = await fetch('/dashboard/agent-dashboard/api/notifications/', {
        ...
        results.forEach((n) => {
            ...
            item.innerHTML = `
                <a href="${url}" style="text-decoration:none; color:inherit; display:flex; gap:10px;" data-notification-id="${n.id || ''}" data-notification-type="${n.category || ''}">
...
window.__refreshAgentNotifications = refreshAgentNotifications;

// Mark individual notification as read
async function markNotificationAsRead(category, notificationId) {
    try {
        let url = '';
        if (category === 'system' && notificationId.startsWith('chat_')) {
            // Mark chat message as read
            const chatId = notificationId.replace('chat_', '');
            url = `/dashboard/agent-dashboard/api/mark-chat-read/${chatId}/`;
        } else if (category === 'tickets' && notificationId.startsWith('ticket_')) {
            // For tickets, we'll just refresh the notifications since ticket status represents "read" state
